{-# LANGUAGE DeriveGeneric #-}
{-# LANGUAGE DerivingStrategies #-}
{-# LANGUAGE LambdaCase #-}
{-# LANGUAGE OverloadedStrings #-}
{-# OPTIONS_GHC -Wno-unused-imports #-}

-- | This module has been GENERATED by fix-codegen.
-- Any manual edits will be undone the next time fix-codegen is run.
module FIX.Fields.UnderlyingSecurityAltID where

import Control.Monad
import Data.ByteString (ByteString)
import Data.Proxy
import Data.Validity
import FIX.Core
import GHC.Generics (Generic)

{-# ANN module ("HLint: ignore" :: String) #-}

-- | FieldSpec
--   { fieldNumber = 458
--   , fieldName = "UnderlyingSecurityAltID"
--   , fieldType = FieldTypeString
--   , fieldValues = []
--   }
newtype UnderlyingSecurityAltID = UnderlyingSecurityAltID {unUnderlyingSecurityAltID :: SimpleBytes}
  deriving stock (Show, Eq, Generic)

instance Validity UnderlyingSecurityAltID

instance IsField UnderlyingSecurityAltID where
  fieldTag Proxy = 458
  fieldIsData Proxy = False
  fieldToValue = toValue . unUnderlyingSecurityAltID
  fieldFromValue = fromValue >=> (prettyValidate . UnderlyingSecurityAltID)
