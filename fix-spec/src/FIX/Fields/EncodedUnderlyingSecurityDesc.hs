{-# LANGUAGE DeriveGeneric #-}
{-# LANGUAGE DerivingStrategies #-}
{-# LANGUAGE LambdaCase #-}
{-# LANGUAGE OverloadedStrings #-}
{-# OPTIONS_GHC -Wno-unused-imports #-}

-- | This module has been GENERATED by fix-codegen.
-- Any manual edits will be undone the next time fix-codegen is run.
module FIX.Fields.EncodedUnderlyingSecurityDesc where

import Control.Monad
import Data.ByteString (ByteString)
import Data.Proxy
import Data.Validity
import FIX.Core
import GHC.Generics (Generic)

{-# ANN module ("HLint: ignore" :: String) #-}

-- | FieldSpec
--   { fieldNumber = 365
--   , fieldName = "EncodedUnderlyingSecurityDesc"
--   , fieldType = FieldTypeData
--   , fieldValues = []
--   }
newtype EncodedUnderlyingSecurityDesc = EncodedUnderlyingSecurityDesc {unEncodedUnderlyingSecurityDesc :: DataBytes}
  deriving stock (Show, Eq, Generic)

instance Validity EncodedUnderlyingSecurityDesc

instance IsField EncodedUnderlyingSecurityDesc where
  fieldTag Proxy = 365
  fieldIsData Proxy = True
  fieldToValue = toValue . unEncodedUnderlyingSecurityDesc
  fieldFromValue = fromValue >=> (prettyValidate . EncodedUnderlyingSecurityDesc)
