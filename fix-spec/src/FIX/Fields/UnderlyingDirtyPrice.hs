{-# LANGUAGE DeriveGeneric #-}
{-# LANGUAGE DerivingStrategies #-}
{-# LANGUAGE LambdaCase #-}
{-# LANGUAGE OverloadedStrings #-}
{-# OPTIONS_GHC -Wno-unused-imports #-}

-- | This module has been GENERATED by fix-codegen.
-- Any manual edits will be undone the next time fix-codegen is run.
module FIX.Fields.UnderlyingDirtyPrice where

import Control.Monad
import Data.ByteString (ByteString)
import Data.Proxy
import Data.Validity
import FIX.Core
import GHC.Generics (Generic)

{-# ANN module ("HLint: ignore" :: String) #-}

-- | FieldSpec
--   { fieldNumber = 882
--   , fieldName = "UnderlyingDirtyPrice"
--   , fieldType = FieldTypePrice
--   , fieldValues = []
--   }
newtype UnderlyingDirtyPrice = UnderlyingDirtyPrice {unUnderlyingDirtyPrice :: SimpleBytes}
  deriving stock (Show, Eq, Generic)

instance Validity UnderlyingDirtyPrice

instance IsField UnderlyingDirtyPrice where
  fieldTag Proxy = 882
  fieldIsData Proxy = False
  fieldToValue = toValue . unUnderlyingDirtyPrice
  fieldFromValue = fromValue >=> (prettyValidate . UnderlyingDirtyPrice)
