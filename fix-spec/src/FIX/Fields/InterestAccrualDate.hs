{-# LANGUAGE DeriveGeneric #-}
{-# LANGUAGE DerivingStrategies #-}
{-# LANGUAGE LambdaCase #-}
{-# LANGUAGE OverloadedStrings #-}
{-# OPTIONS_GHC -Wno-unused-imports #-}

-- | This module has been GENERATED by fix-codegen.
-- Any manual edits will be undone the next time fix-codegen is run.
module FIX.Fields.InterestAccrualDate where

import Control.Monad
import Data.ByteString (ByteString)
import Data.Proxy
import Data.Validity
import FIX.Core
import GHC.Generics (Generic)

{-# ANN module ("HLint: ignore" :: String) #-}

-- | FieldSpec
--   { fieldNumber = 874
--   , fieldName = "InterestAccrualDate"
--   , fieldType = FieldTypeLocalMktDate
--   , fieldValues = []
--   }
newtype InterestAccrualDate = InterestAccrualDate {unInterestAccrualDate :: LocalMktDate}
  deriving stock (Show, Eq, Generic)

instance Validity InterestAccrualDate

instance IsField InterestAccrualDate where
  fieldTag Proxy = 874
  fieldIsData Proxy = False
  fieldToValue = toValue . unInterestAccrualDate
  fieldFromValue = fromValue >=> (prettyValidate . InterestAccrualDate)
