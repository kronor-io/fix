{-# LANGUAGE DeriveGeneric #-}
{-# LANGUAGE DerivingStrategies #-}
{-# LANGUAGE LambdaCase #-}
{-# LANGUAGE OverloadedStrings #-}
{-# OPTIONS_GHC -Wno-unused-imports #-}

-- | This module has been GENERATED by fix-codegen.
-- Any manual edits will be undone the next time fix-codegen is run.
module FIX.Fields.UnderlyingCreditRating where

import Control.Monad
import Data.ByteString (ByteString)
import Data.Proxy
import Data.Validity
import FIX.Core
import GHC.Generics (Generic)

{-# ANN module ("HLint: ignore" :: String) #-}

-- | FieldSpec
--   { fieldNumber = 256
--   , fieldName = "UnderlyingCreditRating"
--   , fieldType = FieldTypeString
--   , fieldValues = []
--   }
newtype UnderlyingCreditRating = UnderlyingCreditRating {unUnderlyingCreditRating :: SimpleBytes}
  deriving stock (Show, Eq, Generic)

instance Validity UnderlyingCreditRating

instance IsField UnderlyingCreditRating where
  fieldTag Proxy = 256
  fieldIsData Proxy = False
  fieldToValue = toValue . unUnderlyingCreditRating
  fieldFromValue = fromValue >=> (prettyValidate . UnderlyingCreditRating)
