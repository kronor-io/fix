{-# LANGUAGE DeriveGeneric #-}
{-# LANGUAGE DerivingStrategies #-}
{-# LANGUAGE LambdaCase #-}
{-# LANGUAGE OverloadedStrings #-}
{-# OPTIONS_GHC -Wno-unused-imports #-}

-- | This module has been GENERATED by fix-codegen.
-- Any manual edits will be undone the next time fix-codegen is run.
module FIX.Fields.UnderlyingRepurchaseRate where

import Control.Monad
import Data.ByteString (ByteString)
import Data.Proxy
import Data.Validity
import FIX.Core
import GHC.Generics (Generic)

{-# ANN module ("HLint: ignore" :: String) #-}

-- | FieldSpec
--   { fieldNumber = 245
--   , fieldName = "UnderlyingRepurchaseRate"
--   , fieldType = FieldTypePercentage
--   , fieldValues = []
--   }
newtype UnderlyingRepurchaseRate = UnderlyingRepurchaseRate {unUnderlyingRepurchaseRate :: SimpleBytes}
  deriving stock (Show, Eq, Generic)

instance Validity UnderlyingRepurchaseRate

instance IsField UnderlyingRepurchaseRate where
  fieldTag Proxy = 245
  fieldIsData Proxy = False
  fieldToValue = toValue . unUnderlyingRepurchaseRate
  fieldFromValue = fromValue >=> (prettyValidate . UnderlyingRepurchaseRate)
