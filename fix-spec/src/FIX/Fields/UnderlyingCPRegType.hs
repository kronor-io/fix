{-# LANGUAGE DeriveGeneric #-}
{-# LANGUAGE DerivingStrategies #-}
{-# LANGUAGE LambdaCase #-}
{-# LANGUAGE OverloadedStrings #-}
{-# OPTIONS_GHC -Wno-unused-imports #-}

-- | This module has been GENERATED by fix-codegen.
-- Any manual edits will be undone the next time fix-codegen is run.
module FIX.Fields.UnderlyingCPRegType where

import Control.Monad
import Data.ByteString (ByteString)
import Data.Proxy
import Data.Validity
import FIX.Core
import GHC.Generics (Generic)

{-# ANN module ("HLint: ignore" :: String) #-}

-- | FieldSpec
--   { fieldNumber = 878
--   , fieldName = "UnderlyingCPRegType"
--   , fieldType = FieldTypeString
--   , fieldValues = []
--   }
newtype UnderlyingCPRegType = UnderlyingCPRegType {unUnderlyingCPRegType :: SimpleBytes}
  deriving stock (Show, Eq, Generic)

instance Validity UnderlyingCPRegType

instance IsField UnderlyingCPRegType where
  fieldTag Proxy = 878
  fieldIsData Proxy = False
  fieldToValue = toValue . unUnderlyingCPRegType
  fieldFromValue = fromValue >=> (prettyValidate . UnderlyingCPRegType)
